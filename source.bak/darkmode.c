...
void darkmodeUpdate(const AppInput* in, int* currentScreen) {
    if (s_hexEditing) {
        colorPickerInput(&s_hexPicker, in->down);
        if (in->confirm) {
    ...
    if (in->touchDown || in->touchHeld) {
        float segX = 60.0f, segY = 8.0f, segW = 100.0f, segH = 28.0f;
...
void darkmodeRenderBottom(C2D_TextBuf buf, float transVal) {
    ...
    if (s_segSlideT < 1.0f) {
        s_morphX = approach(s_morphX, targetX, 4.0f);
    } else {
        s_morphX = targetX;
    }
    ...
    UI_HelpBar(buf, "A aplicar  <- -> accent", "START sair");
}
