    ...
    if (!dragging && !modeChanged) {
        ...
        if (s_mode == LED_SOLID || s_mode == LED_PULSE) {
            if (s_selected >= 1 && s_selected <= 3) {
                ...
                if (in->left) { *ch = (u8)clampi((int)*ch - 8, 0, 255); applyLed(); if (s_savePending == 0) s_savePending = SAVE_DEBOUNCE_FRAMES; }
                if (in->right) { *ch = (u8)clampi((int)*ch + 8, 0, 255); applyLed(); if (s_savePending == 0) s_savePending = SAVE_DEBOUNCE_FRAMES; }
            }
            if (s_selected == 4) {
...
void ledRenderBottom(C2D_TextBuf buf, float transVal) {
    ...
    if (s_mode == LED_SOLID || s_mode == LED_PULSE) {
        ...
        float swX = 142.0f, swY = 56.0f + offset;
        UI_RoundRect(swX, swY, 28, 28, 14, pc);
        ColorRGBA pcGlow = pc;
        pcGlow.a = 50;
        UI_RoundRect(swX - 2, swY - 2, 32, 32, 16, pcGlow);
    } else {
