...
void uiFrameTick(void) {
    ...
    g_frame = s_frame;
    if (g_enterT < 1.0f) g_enterT = approach(g_enterT, 1.0f, 0.035f);
    if (g_trans.active) transitionUpdate(&g_trans);
}
...
void popupUpdate(PopupModal* p) {
    if (!p->active) return;
    p->animT = fminf(p->animT + 1.0f / 60.0f, 0.35f);
...
void UI_TouchBarSegmented(C2D_TextBuf buf, float x, float y, float w, float h,
    ...
    if (morphT < 1.0f) {
        ...
        s_morphX = approach(s_morphX, targetV, 4.0f);
    } else {
        s_morphX = targetX;
    }
