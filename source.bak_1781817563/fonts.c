...
static C2D_Font previewFontFor(int index) {
    if (index < 0 || index >= MAX_CUSTOM_FONTS) return NULL;
    return fontsGetFont(index);
}
...
static void drawFontPreviewBars(C2D_TextBuf buf, C2D_Font font, int index, float x, float y, float t) {
    ...
    UI_Text(buf, font, previewLine(index), x + 18, y + 18 + slide,
            (index == 2 || index == 3) ? 0.40f : 0.36f,
            (index == 1 || index == 3) ? 0.43f : 0.36f,
            g_theme.textPrimary);
    UI_Text(buf, font, "Aa Bb Cc 123", x + 18, y + 43 + slide,
...
void fontsRenderTop(C2D_TextBuf buf) {
    ...
    C2D_Font previewFont = previewFontFor(s_selected);
    UI_Text(buf, NULL, "Preview de Fonte", 32, 48 + slideUp, 0.38f, 0.38f, g_theme.textSecondary);
    UI_Text(buf, previewFont, previewTitle(s_selected), 32, 68 + slideUp, 0.58f, 0.58f, g_theme.textPrimary);
    ...
    drawFontPreviewBars(buf, previewFont, s_selected, 32, 106 + slideUp, s_previewT);
