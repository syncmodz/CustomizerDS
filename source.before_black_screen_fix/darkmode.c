...
void darkmodeRenderBottom(C2D_TextBuf buf) {
    ...
    float targetY = (s_selected == 0) ? 42.0f : (s_selected == 1) ? 42.0f : 92.0f + ((s_selected - 2) / 2) * 36.0f;
    s_selY += (targetY - s_selY) * 0.18f;
    UI_RoundRect(8, s_selY, 304, (s_selected < 2) ? 34 : 30, 4, themeMix(g_theme.surface, g_theme.primary, 0.12f));
