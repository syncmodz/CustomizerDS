...
void UI_CapsuleButton(C2D_TextBuf buf, C2D_Font font, float x, float y, float w, float h,
    ...
    if (state == UI_BUTTON_SELECTED) {
        ...
        float amt = 0.16f + pulse - pressedT * 0.06f;
    ...
    if (value) {
        UI_TextRight(buf, NULL, value, ox + w - 12, oy + (h - 16) * 0.5f, 0.28f, 0.28f, g_theme.textPrimary);
    }
}
...
void UI_KeyHelp(C2D_TextBuf buf, C2D_Font font, const char* left, const char* right) {
    UI_Fill(0, SCREEN_BOT_HEIGHT - 24, SCREEN_BOT_WIDTH, 24,
            (ColorRGBA){0, 0, 0, 60});
    if (left)
        UI_Text(buf, font, left, 9, SCREEN_BOT_HEIGHT - 18, 0.24f, 0.24f, g_theme.textSecondary);
    if (right)
        UI_TextRight(buf, font, right, SCREEN_BOT_WIDTH - 9, SCREEN_BOT_HEIGHT - 18, 0.24f, 0.24f, g_theme.textSecondary);
}
...
void UI_Badge(C2D_TextBuf buf, C2D_Font font, float x, float y, const char* text, ColorRGBA color) {
    ...
    ColorRGBA badgeText = ((int)color.r + (int)color.g + (int)color.b > 430)
        ...
        : g_theme.onPrimary;
    UI_RoundRect(x, y, pw, 20, 10, color);
    UI_TextCenter(buf, NULL, text, x + pw * 0.5f, y + 3, 0.24f, 0.24f, badgeText);
}
...
void UI_StartupLogo(C2D_TextBuf buf, float t) {
    ...
    if (t > 0.1f) {
        ...
        UI_TextCenter(buf, NULL, "CDS", lx, ly - 6, 0.60f, 0.60f, g_theme.onPrimary);
