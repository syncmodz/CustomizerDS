...
void ledRenderTop(C2D_TextBuf buf) {
    ...
    UI_Text(buf, fontsCurrent(), MODE_NAMES[s_mode], 32, 52 + slideUp, 0.52f, 0.52f, g_theme.textPrimary);
...
static void ledSliderRow(C2D_TextBuf buf, const char* label, float x, float y, float w,
    ...
    ColorRGBA bg = selected ? themeMix((ColorRGBA){18, 22, 31, 246}, g_theme.accent, 0.08f)
                             : (ColorRGBA){18, 22, 31, 246};
...
void ledRenderBottom(C2D_TextBuf buf) {
    ...
    UI_RoundFrame(10, segY, segW, segH, segH * 0.5f,
                  (ColorRGBA){18, 22, 31, 246}, (ColorRGBA){255, 255, 255, 18});
