...
void fontsRenderTop(C2D_TextBuf buf) {
    ...
    UI_Text(buf, previewFont, "CustomizerDS", 32, 48 + slideUp, 0.74f, 0.74f, g_theme.textPrimary);
    UI_Text(buf, previewFont, "Aa Bb Cc 123", 32, 88 + slideUp, 0.52f, 0.52f, g_theme.accent);
    UI_Text(buf, previewFont, "font preview", 218, 92 + slideUp, 0.32f, 0.32f, g_theme.textSecondary);
    ...
    UI_Badge(buf, NULL, 32, 126 + slideUp, status, badgeC);

    char nameBuf[64];
    snprintf(nameBuf, sizeof(nameBuf), "%s", fontsLabel(s_selected));
    UI_Text(buf, NULL, nameBuf, 32, 154 + slideUp, 0.31f, 0.31f, g_theme.textPrimary);

    float previewT = clampf(et * 2.0f, 0.0f, 1.0f);
    float py = lerpf(186 + slideUp, 181 + slideUp, previewT);
    UI_Text(buf, NULL, loaded ? "fonte real carregada via romfs" : "usando fonte padrao do sistema",
            32, py, 0.24f, 0.24f, loaded ? g_theme.textSecondary : g_theme.warning);
}
...
void fontsRenderBottom(C2D_TextBuf buf) {
    ...
    for (int i = 0; i < fontsCount(); i++) {
        ...
        C2D_Font rowFont = NULL;
        if (i == s_selected || i == g_fonts.currentIndex) {
            rowFont = fontsGetFont(i);
        }
        UI_CapsuleButton(buf, rowFont, 10, by, 300, 34,
                         fontsLabel(i),
...
C2D_Font fontsCurrent(void) {
    if (g_fonts.current == NULL && g_fonts.currentIndex >= 0 && g_fonts.currentIndex < MAX_CUSTOM_FONTS) {
        g_fonts.current = fontsGetFont(g_fonts.currentIndex);
    }
