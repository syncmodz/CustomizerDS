...
void UI_TouchBarBackground(void) {
    UI_Fill(0, 0, SCREEN_BOT_WIDTH, SCREEN_BOT_HEIGHT, (ColorRGBA){3, 4, 6, 255});
    UI_Fill(0, 0, SCREEN_BOT_WIDTH, 42, (ColorRGBA){255, 255, 255, 5});
    UI_Fill(0, SCREEN_BOT_HEIGHT - 34, SCREEN_BOT_WIDTH, 34, (ColorRGBA){0, 0, 0, 34});
    UI_Line(0, 0, SCREEN_BOT_WIDTH, 1, (ColorRGBA){255, 255, 255, 14});
}
...
void UI_CapsuleButton(C2D_TextBuf buf, C2D_Font font, float x, float y, float w, float h,
    ...
    ColorRGBA bg = {18, 22, 31, 246};
    ColorRGBA labelColor = g_theme.textPrimary;
    ColorRGBA borderCol = {255, 255, 255, 18};
    ColorRGBA iconBg = {36, 45, 60, 235};

    if (state == UI_BUTTON_SELECTED) {
        float pulse = sinf(s_frame * 3.5f) * 0.03f;
        float amt = 0.08f + pulse - pressedT * 0.03f;
        bg = themeMix((ColorRGBA){18, 22, 31, 246}, g_theme.accent, amt);
        labelColor = g_theme.textPrimary;
        borderCol = g_theme.accent;
        borderCol.a = 74;
        iconBg = themeMix((ColorRGBA){36, 45, 60, 235}, g_theme.accent, 0.35f);
    } else if (state == UI_BUTTON_ACTIVE) {
        bg = themeMix((ColorRGBA){18, 22, 31, 246}, g_theme.accent, 0.08f);
        borderCol.a = 30;
    }
