...
static float s_segFromX = 10.0f;
static bool s_ledDirty = false;

#define LED_SEG_X 10.0f
#define LED_SEG_Y 28.0f
#define LED_SEG_W 300.0f
#define LED_SEG_H 28.0f

static float ledModeTargetX(int mode) {
    float itemW = LED_SEG_W / (float)LED_MODE_COUNT;
    return LED_SEG_X + mode * itemW + 3.0f;
}
...
void ledEnter(void) {
    ...
    s_draggingSlider = -1;
    s_pressedT = 0.0f;
    s_segFromX = ledModeTargetX(s_mode);
    s_segSlideT = 1.0f;
    s_ledDirty = false;
}
...
static void setMode(int mode) {
    mode = clampi(mode, 0, LED_MODE_COUNT - 1);
    if (mode != s_mode) {
        s_segFromX = ledModeTargetX(s_mode);
        s_segSlideT = 0.0f;
    }
...
void ledUpdate(const AppInput* in, int* currentScreen) {
    ...
    if (!dragging) {
        ...
        if (s_selected == 0) {
            if (in->left) { setMode((s_mode - 1 + LED_MODE_COUNT) % LED_MODE_COUNT); }
            if (in->right) { setMode((s_mode + 1) % LED_MODE_COUNT); }
        }
    ...
    if (in->touchDown || (in->touchHeld && s_draggingSlider >= 0)) {
        float segX = LED_SEG_X, segY = LED_SEG_Y, segW = LED_SEG_W, segH = LED_SEG_H;
    ...
    if (!in->touchHeld) {
        if (s_draggingSlider >= 0 && s_ledDirty) {
            saveLed();
            s_ledDirty = false;
        }
        s_draggingSlider = -1;
...
void ledRenderTop(C2D_TextBuf buf) {
    ...
    UI_Text(buf, NULL, MODE_NAMES[s_mode], 32, 52 + slideUp, 0.52f, 0.52f, g_theme.textPrimary);
    UI_Text(buf, NULL, s_mcuReady ? "MCU pronto - led ao vivo" : "MCU indisponivel - preview apenas",
            32, 80 + slideUp, 0.25f, 0.25f, s_mcuReady ? g_theme.success : g_theme.warning);

    ColorRGBA c = previewColor();
    float lx = 266.0f, ly = 52 + slideUp, lw = 88, lh = 64;
    UI_RoundFrame(lx, ly, lw, lh, 18, (ColorRGBA){8, 10, 14, 255}, (ColorRGBA){255, 255, 255, 16});
    UI_RoundRect(lx + 6, ly + 6, lw - 12, lh - 12, 8, c);
    ColorRGBA glow = c;
    glow.a = 34;
    UI_RoundRect(lx + 3, ly + 3, lw - 6, lh - 6, 15, glow);

    char rgb[40];
    snprintf(rgb, sizeof(rgb), "R%d G%d B%d  vel %d", s_r, s_g, s_b, s_speed);
    UI_Text(buf, NULL, rgb, 32, 108 + slideUp, 0.29f, 0.29f, g_theme.textSecondary);

    if (R_FAILED(s_lastResult)) {
        char err[32];
        snprintf(err, sizeof(err), "erro: 0x%08lX", (unsigned long)s_lastResult);
        UI_Text(buf, NULL, err, 32, 134 + slideUp, 0.23f, 0.23f, g_theme.textHint);
    } else {
        UI_Text(buf, NULL, "toque nos controles abaixo para ajustar", 32, 134 + slideUp, 0.24f, 0.24f, g_theme.textHint);
    }
    float previewT = clampf(et * 2.0f, 0.0f, 1.0f);
    float py = lerpf(166 + slideUp, 160 + slideUp, previewT);
...
static void ledSliderRow(C2D_TextBuf buf, const char* label, float x, float y, float w,
    ...
    ColorRGBA bg = selected ? themeMix(g_theme.surfaceElevated, g_theme.accent, 0.08f)
                             : g_theme.surfaceElevated;
    ColorRGBA border = selected ? g_theme.accent : g_theme.divider;
    if (selected) border.a = themeIsDark() ? 72 : 118;
    UI_RoundFrame(x, y, w, 26, 13, bg, border);
...
void ledRenderBottom(C2D_TextBuf buf) {
    ...
    float segH = LED_SEG_H, segY = LED_SEG_Y, segW = LED_SEG_W;
    UI_RoundFrame(LED_SEG_X, segY, segW, segH, segH * 0.5f,
                  g_theme.surfaceElevated, g_theme.divider);

    float segItemW = segW / (float)LED_MODE_COUNT;
    float targetX = ledModeTargetX(s_mode);
    ...
    UI_RoundFrame(curX, segY + 2, segItemW - 6, segH - 4, (segH - 4) * 0.5f, selBg, selBorder);
    for (int i = 0; i < LED_MODE_COUNT; i++) {
        ColorRGBA tc = (i == s_mode) ? g_theme.textPrimary : g_theme.textSecondary;
        UI_TextCenter(buf, NULL, MODE_NAMES[i], LED_SEG_X + segItemW * i + segItemW * 0.5f, segY + 6, 0.26f, 0.26f, tc);
    }
